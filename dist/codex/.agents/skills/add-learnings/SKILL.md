---
name: add-learnings
description: "[ADD v0.10.0] Manage learnings — generate active views, archive old entries, show stats"
argument-hint: "[migrate|archive|stats] [--dry-run]"
---

# ADD Learnings Skill v0.10.0
...
1. Run `~/.codex/hooks/filter-learnings.sh` on `.add/learnings.json`
2. Run `~/.codex/hooks/filter-learnings.sh` on `~/.claude/add/library.json` (if it exists)
...
6. Run `~/.codex/hooks/filter-learnings.sh` on the new JSON
