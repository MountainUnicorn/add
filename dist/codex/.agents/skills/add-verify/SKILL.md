---
name: add-verify
description: "[ADD v0.9.10] Run quality gates — lint, types, tests, coverage, spec compliance"
argument-hint: "[--level local|ci|deploy|smoke] [--fix]"
---

# ADD Verify Skill v0.9.10
...
1. **Read .add/config.json**
   ...
   - A configuration example lives in `~/.codex/add/templates/verify-report.md`
...
6. **Include in gate report** — append maturity-scaled results to the gate's report section, and add the Maturity-Scaled Checks Summary to the overall report (formats in `~/.codex/add/templates/verify-report.md`)
