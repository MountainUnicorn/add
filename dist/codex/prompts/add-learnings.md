...
1. Run `~/.codex/add/hooks/filter-learnings.sh` on `.add/learnings.json`
2. Run `~/.codex/add/hooks/filter-learnings.sh` on `~/.claude/add/library.json` (if it exists)
...
6. Run `~/.codex/add/hooks/filter-learnings.sh` on the new JSON
