...
**Fallback** (if `-active.md` doesn't exist): run `~/.codex/hooks/filter-learnings.sh <path-to-json>` and read the result; if the script fails, read the full JSON and apply in-context filtering (cap at 10 by severity). Learnings are never lost — JSON is canonical. A PostToolUse hook regenerates `-active.md` automatically whenever a learnings JSON file is written, keeping filtering out of agent context.
