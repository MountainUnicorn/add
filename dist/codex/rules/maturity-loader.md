...
2. **Fallback path (hooks disabled or failed):** if no `# ADD Rules` block appears in context, Read `.add/config.json` for `maturity.level`, then Read each file in `~/.codex/add/rules/` whose `maturity:` gate is at or below that level (skip `autoload: false` rules — those load on demand via skill `references:`).
