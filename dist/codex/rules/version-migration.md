...
3. **Execute steps** from the manifest for each hop. Supported actions:
   ...
   - `remove_stale_rule_copies` — find files in `.claude/rules/` whose basename matches a plugin rule in `~/.codex/add/rules/` (also `add-` prefixed variants; user-authored files never match). List the matches and ask the user ONCE for confirmation; on yes, back each up per step 2, then delete. NEVER delete without explicit confirmation.
