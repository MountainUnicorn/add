...
| Hooks (learnings filter, CHANGELOG, autofix) | Enforced (PreToolUse / SessionStart) | **Broken on Codex ≥0.14x** — hooks.json schema changed (`unknown field 'SessionStart'`), registration is rejected (#24, fix target v0.10.1) | Codex `hooks.json` merge is manual when one already exists |
...
| Sub-agents (test-writer, implementer, reviewer, explorer) | Enforced (Task tool dispatch) | **Broken on Codex ≥0.14x** — agent TOMLs rejected (`unknown field 'prompt_skill'`), silently ignored (#24, fix target v0.10.1) | Worked on the old 0.122 pin; caught by the re-pin smoke |
