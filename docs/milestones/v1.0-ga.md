...
- **#1**: release.sh now refuses to tag unless HEAD's CI checks are green
  (`--no-verify-ci` emergency override prints loudly); branch protection with
  required checks to be applied once the smoke workflows exist on main.
- **#2**: `install-smoke-claude.yml` + `install-smoke-codex.yml` built.
  Codex smoke **passed locally in Docker against the re-pinned CLI 0.144.5**
  (Q-001 re-baseline done — 27/27 skills install; agent-driven leg runs in CI
  with secrets). Claude smoke pending first CI run.
