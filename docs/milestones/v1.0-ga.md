...
- **#2 MET (layout legs)**: `install-smoke-claude.yml` + `install-smoke-codex.yml`
  green in CI on first run. Codex smoke also passed locally in Docker against
  the re-pinned CLI 0.144.5 (Q-001 re-baseline done — 27/27 skills install).
  Agent-driven legs (/add:init, /add-init) currently SKIP with a warning:
  repo has no ANTHROPIC_API_KEY / OPENAI_API_KEY Actions secrets — add them
  to fully close #2.
