# Telemetry Reference — Structured Telemetry (JSONL)

> Full spec for the ADD telemetry write path. Loaded on demand by skills that
> emit telemetry lines. The condensed invariant lives in `rules/telemetry.md`.
...
Every ADD SKILL.md declares `rules/telemetry.md` in its `references:` frontmatter, which resolves to the condensed rule and this reference.
